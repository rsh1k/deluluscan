...
namespace**. It touches nothing belonging to other tenants (e.g. a peer service) — it
only *borrows the pattern* a peer service established (EKS + ALB + ECR on the corp
account).

## Why this design (the researched choice)

a peer service authenticates with **app-level Google OAuth** (its FastAPI app owns the
...
| App-level OAuth (like a peer service) | ❌ Needs an app; overkill for a static file |
...
1. **kubectl access to the corp EKS cluster** (a peer service uses context
   `<EKS_CONTEXT>`), via SDM. — *platform team*
2. **An ECR repo** for the image, e.g. `<ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/deluluscan/dashboard`,
   plus push access. a peer service's account is `<ACCOUNT_ID>`. — *platform team (Terraform)*
3. **A Google OAuth client** (id + secret) from the target Google Workspace /
   ...
   `https://dashboard.example.com/oauth2/idpresponse`. — *Workspace/security admin*
4. **DNS**: a Route53 record `dashboard.example.com` → the ALB. — *platform team*
5. **ACM certificate** for `dashboard.example.com` (or reuse a `*.example.com` wildcard). — *platform team*
6. *(optional)* an **IP-allowlist SG** on the ALB, mirroring a peer service. — *platform team*
...
- **CI (automatic):** `.github/workflows/deploy-dashboard.yml` rebuilds the image,
  ...
  Deluluscan-owned IAM role, never a peer service's. Once wired, the nightly scan can
